...
export default function ReportModal({ open, onClose, onSubmit }: Props) {
  ...
  return (
    ...
    >
      <div className="relative mx-auto flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-3xl border border-red-950/50 bg-[#080808] shadow-[0_40px_120px_rgba(0,0,0,0.95)]">

        {/* Header */}
        <div className="flex items-center justify-between border-b border-red-950/30 bg-[#0a0808] px-6 py-4">
          <div>
            <p className="font-mono text-[9px] uppercase tracking-[0.5em] text-amber-400/60">J.A.S. // FIELD REPORT // CLASSIFIED</p>
            <h2 className="mt-1 font-mono text-lg font-bold tracking-tight text-white">Submit Sighting</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-1 flex-col overflow-hidden">
          <div className="flex flex-1 overflow-hidden">
            ...
            <div className="flex flex-1 flex-col gap-4 overflow-y-auto p-6">
                    ...
                    <ul className="absolute left-0 right-0 top-full z-50 mt-1 overflow-hidden rounded-xl border border-slate-700/50 bg-[#0f0f12] shadow-2xl">
                      {suggestions.map((loc) => (
                        <li key={loc} onMouseDown={() => { setLocationName(loc); setShowSug(false); }}
                          className="cursor-pointer px-4 py-2 font-mono text-xs text-slate-300 transition hover:bg-red-950/40 hover:text-white">
                          {loc}
                        </li>
                      ))}
                    </ul>
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {LOCATIONS.map((loc) => (
                    <button key={loc} type="button" onClick={() => { setLocationName(loc); setShowSug(false); }}
                      className="rounded-lg border border-slate-800/50 bg-slate-900/60 px-2 py-1 font-mono text-[9px] uppercase tracking-widest text-slate-400 transition hover:border-red-900/50 hover:text-red-300">
                      {loc}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <textarea value={description} onChange={(e) => setDescription(e.target.value.slice(0, 280))}
                  placeholder="Describe direction of movement, behavior, any identifying details..."
                  rows={3}
                  className="w-full resize-none rounded-xl border border-slate-700/50 bg-[#101013] px-4 py-2.5 font-mono text-sm text-white placeholder-slate-700 outline-none focus:border-red-800/60" />
              </div>
              ...
              <div>
                ...
                <div onClick={() => fileRef.current?.click()}
                  className="flex min-h-[72px] cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-slate-700/40 bg-[#0d0d10] transition hover:border-red-900/40">
                  {photoPreview
            ...
            <div className="flex w-72 flex-shrink-0 flex-col border-l border-slate-800/40 bg-[#070709]">
              ...
              <div className="border-t border-slate-800/40 px-4 py-3">
                <p className="font-mono text-[9px] tabular-nums text-amber-400/60">
                  {pinned ? `${lat.toFixed(5)}, ${lng.toFixed(5)}` : 'No pin placed — using campus center'}
                </p>
              </div>
            </div>
          </div>
          ...
          {/* Footer */}
          <div className="flex gap-3 border-t border-red-950/30 px-6 py-4">
            <button type="button" onClick={onClose}
              className="flex-1 rounded-xl border border-slate-700/40 px-4 py-2.5 font-mono text-xs uppercase tracking-widest text-slate-500 transition hover:text-white">
              Abort
            </button>
            <button type="submit" disabled={submitting}
              className="flex-[2] rounded-xl bg-red-600 px-4 py-2.5 font-mono text-xs font-bold uppercase tracking-[0.22em] text-white shadow-lg transition hover:bg-red-500 disabled:opacity-40">
